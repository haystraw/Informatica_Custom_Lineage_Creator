...
def findSourceMatchedDatasets(source_resource, source_dataset_regex):
    source_dataset_match = df_full_export_Datasets[
        df_full_export_Datasets[dataset_hierarchical_column].str.startswith(f"{source_resource}/") &
        df_full_export_Datasets[dataset_name_column].str.contains('^'+source_dataset_regex+'$', na=False, regex=True, flags=re.IGNORECASE)
    ]
    return source_dataset_match

def findTargetMatchedDatasets(source_dataset_path, source_dataset_refid, source_dataset_name, target_resource, target_dataset_regex, dataset_score_threshold):

    resulting_dataframe = None
    target_dataset_match = df_full_export_Datasets[
        df_full_export_Datasets[dataset_hierarchical_column].str.startswith(f"{target_resource}/", na=False) &
        df_full_export_Datasets[dataset_name_column].str.contains('^'+target_dataset_regex.replace('{name}',source_dataset_name)+'$', na=False, regex=True, flags=re.IGNORECASE)
    ]

    for index, target in target_dataset_match.iterrows():
        target_dataset_path = target[dataset_hierarchical_column]
        target_dataset_refid = target[dataset_refid_column]
        target_dataset_name = target[dataset_name_column]

        this_score = difflib.SequenceMatcher(None, source_dataset_name, target_dataset_name).ratio()
        
        if this_score >= float(dataset_score_threshold):
            this_dataset_lineage = {
                            'src Reference ID': source_dataset_refid, 
                            'src HierarchicalPath': source_dataset_path, 
                            'src Name': source_dataset_name,
                            'tgt Reference ID': target_dataset_refid, 
                            'tgt HierarchicalPath': target_dataset_path, 
                            'tgt Name': target_dataset_name,
                            'Association': 'core.DataSetDataFlow',
                            'Match Score': this_score
                            }
            
            df = pandas.DataFrame(this_dataset_lineage, index=[0])
            resulting_dataframe = append_or_create(resulting_dataframe,df)
    return resulting_dataframe


def findSourceMatchedElements(source_resource, source_dataset_regex):
...
def readConfigAndStart(fileName):
    ...
    with open(fileName) as csv_file:
        ...
        for row in csv_reader:
            ...
            source_dataset_match = findSourceMatchedDatasets(this_Source_Resource,this_Source_Dataset_regex)
            ...
            for index, source in source_dataset_match.iterrows():
                ...
                dataset_dataframe = findTargetMatchedDatasets(ds_hp, ds_id, ds_name, this_Target_Resource, this_Target_Dataset_regex, this_Dataset_Match_Score)
                final_dataframe = append_or_create(final_dataframe,dataset_dataframe)
