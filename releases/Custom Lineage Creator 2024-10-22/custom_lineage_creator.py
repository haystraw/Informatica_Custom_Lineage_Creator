...
def find_latest_xlsx():
    # Find all .zip and .xlsx files in the current directory
    files = glob.glob("*.zip") + glob.glob("*.xlsx")
    
    if not files:
        print("No ZIP or XLSX files found.")
        exit
    
    latest_file = max(files, key=os.path.getmtime)
    
    return latest_file
export_file = find_latest_xlsx()
...
def write_csv(df):
    ...
    file_name = 'links_'+timestamp+'.csv'
